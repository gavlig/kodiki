...
impl HelixApp {
	...
	pub fn window_title(&self) -> String {
		...
		if let Ok(cwd) = std::env::current_dir() {
			// don't show the whole path, just the last directory in it
			if let Some(cwd_dir) = cwd.file_name() {
				if let Some(cwd_str) = cwd_dir.to_str() {
					title.push_str(" - ");
		...
		title.push_str("] - Code Editor - Kodiki");
