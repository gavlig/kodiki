...
fn handle_camera_mode(
	...
	let fly_mode_keys_pressed = key.pressed(KeyCode::LControl) && key.pressed(KeyCode::LAlt) && key.just_pressed(KeyCode::Home) && keys_pressed == 3;
	...
	match app_context.0 {
		AppContext::CodeEditor => {
			...
			if helix_app.should_render() {
				if let Some(path) = helix_app.current_path() {
		...
		AppContext::Terminal => {
			for terminal in q_terminal.iter() {
				if terminal.active() && terminal.state_changed() {
					window.title = format!("Terminal [{}]", terminal.get_title());
				}
			}
		}
	}
