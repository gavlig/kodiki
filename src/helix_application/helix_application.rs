...
use helix_term      :: { config::Config, job :: Jobs, args::Args };
use serde_json      :: { json };
...
pub struct Application {
    // compositor: Compositor, // helix-term // this is what we replace
    pub editor  : Editor,
...
impl Application {
    pub fn new(args: Args, config: Config) -> Result<Self, Error> {
        ...
        // let mut compositor = Compositor::new().context("build compositor")?; // TODO
        let config = Arc::new(ArcSwap::from_pointee(config));
        let mut editor = Editor::new(
            // compositor.size(),
            helix_view::graphics::Rect::default(), // TODO
            theme_loader.clone(),
            syn_loader.clone(),
